...
function Detail({userName, quizList}) {
  ...
  return (
    ...
    <div className="w-full flex items-center h-full">
      <div className="mx-auto basis-11/12 lg:10/12 flex-wrap items-center h-full">
        ...
        <div className="basis-full text-center">
          {
            ...
            {/* % 게이지 */}
            <p className="absolute top-0 right-3 ">{currentPer}%</p>

              <div className="absolute top-0 left-0 h-full bg-pink-300 rounded-full transition-all duration-1000" style={{width: `${currentPer}%`}}></div>
            </div>
            ...
            <div className="flex items-center mt-4">
              ...
              오답일경우정답 : <span className="bg-green-300 w-5 h-5 block mr-5 ml-2"></span>
              ...
              return (
                <ul key={i} className='mt-5 bg-white rounded-2xl'>
                  <li className="flex justify-between flex-wrap">
                    <p className="bg-gray-50 fount-bold basis-full border text-base py-4 rounded-lg">{e.question}</p>
                    {
                      Object.entries(e.view).map((el,index)=>{
                        return(
                          <p key={index} className={`fount-bold mt-5 basis-[49.5%] border text-base py-4 rounded-lg ${e.answer === el[1] && userAnswer[i]===e.answer ? 'bg-orange-500' : e.answer === el[1] ?
                          'bg-green-300' : el[1]=== userAnswer[i] ? 'bg-indigo-500' : 'bg-whith'}`}>{el[1]}</p>
                        )
                      })
                    }
                  </li>
